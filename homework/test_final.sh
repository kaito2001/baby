...
babyd keys add test2 --keyring-backend $KEYRING --algo $KEYALGO --output json | jq -r '.mnemonic'
...
# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
babyd start --pruning=nothing --log_level $LOGLEVEL --minimum-gas-prices=0.0001ufinal --p2p.laddr tcp://0.0.0.0:2204 --rpc.laddr tcp://0.0.0.0:2202 --grpc.address 0.0.0.0:2203 --grpc-web.address 0.0.0.0:2283
